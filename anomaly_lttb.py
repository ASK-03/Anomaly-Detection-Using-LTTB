...
def largest_triangle_three_buckets(data, threshold):
    ...
    for i in range(0, threshold - 2):
        ...
        #####################
        #print(prev_avg_area)
        if area >= prev_avg_area*3\
            and (max_area_point[1]>math.fabs(avg) or max_area_point[1]<math.fabs(avg)*-1):
            anomaly_points.append(max_area_point)
            #print(i, prev_avg_area, prev_area, area)

        avg_area=avg_area/every
        prev_avg_area=avg_area
        avg=(avg+prev_avg)/2
        prev_avg=avg_y
        ####################
